...
# from delight.modules import FactorizedEncoderLayer, FactorizedMultiHeadAttention
...
class MultiHeadedAttentionWithRelations(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.h = h # 4
        self.linears = clones_share(lambda: nn.Linear(d_model, d_model), 4) # (100, 100), 4
        self.attn = None
...
class Encoder(nn.Module):
    def __init__(self, layer, N, initializer_range, tie_layers=False):
        ...
        if tie_layers:
            ...
            self.layers = clones_share(layer, N) # lambda: trm.EncoderLayer(.), 3
...
class EncoderLayer(nn.Module):
    def __init__(self, size, self_attn, feed_forward, num_relation_kinds, dropout):
        ...
        self.sublayer = clones_share(lambda: SublayerConnection(size, dropout), 2) # (100, 0.2), 2
        self.size = size # 100
