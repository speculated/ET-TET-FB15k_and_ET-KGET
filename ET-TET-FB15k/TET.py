...
class TET(nn.Module):
    def __init__(self, args, num_entities, num_rels, num_types, num_cluster):
        ...
        self.use_cuda = True
        self.dataset = 'FB15kET'
        # self.sample_ent2pair_size = args['sample_ent2pair_size'] # 6
        ...
        self.relation = nn.Parameter(torch.randn(self.num_rels, self.embedding_dim)) # (1345 + 1081, 100)
        ...
        '''
        self.pair_layer = args['pair_layer'] # 3
        self.pair_head = args['pair_head'] # 4
        self.pair_dropout = args['pair_dropout'] # 0.2
        self.pair_ff_dim = args['pair_ff_dim'] # 480
    ...
    def forward(self, et_content, kg_content, flag): # sample_et_content_cluster, sample_kg_content_relation,
        ...
        # print('et_content: ' + str(et_content[:, :, 2].siz[Ma9e()))
        ...
        # shape(128 * 3, 3, 100)
        # shape(1, 100).expand(et_merge.size(0), 1, self.embedding_d[Ma?im) -> shape(128 * 3, 1, 100)
        ...
        if self.tt_ablation == 'all':
            ...
            et_kg_merge_et = torch.stack([et_relations, et_types], dim=2) # [et_relations, et_types]
            et_kg_merge_kg = torch.stack([kg_relations, kg_entities], dim=2) # [kg_relations, kg_entities]
        ...
        # shape(128, 1, 100)
        et_kg_merge = self.bert_encoder(et_kg_merge, self.convert_mask(et_kg_merge.new_ones(et_merge.size(0), et_kg_merge.size(1), dtype=torch.long)), output_all_encoded_layers=False)[-1]
        if flag:
            # et_type_list_tag = torch.zeros(batch_size, et_neighbor_size, train_type_label.size(1)).cuda()
            # et_type_list_tag.scatter_(2, (et_content[:, :, 2] - self.num_entities).unsqueeze(2), 1)
            # kg_entity_list_tag = train_type_label[kg_content[:, :, 2]].cuda()
            # et_kg_mask = torch.matmul(et_type_list_tag, kg_entity_list_tag.permute(0, 2, 1))
            ...
            # et_kg_merge_entity_all = et_kg_merge_entity_all.unsqueeze(1).expand(-1, 3, -1, -1)
            # et_kg_merge_entity_select = et_kg_merge_entity_all * et_kg_mask.unsqueeze(-1).expand(-1, -1, -1, self.embedding_dim)
            # et_kg_merge_entity_select = torch.mean(et_kg_merge_entity_select, dim=1)

            et_kg_merge_type = torch.empty(0, device=self.device)
            # et_kg_merge_type_all = torch.empty(0, device=self.device)
            ...
            #     if et_kg_merge[:, l] is None:
            #         break
            #     et_kg_merge_type_all = torch.cat([et_kg_merge_type_all, et_kg_merge[:, l].unsqueeze(1)], dim=1)
            ...
            # for k in range(et_neighbor_size * 2 + 2, et_kg_size + 1, 2):
            #     if et_kg_merge[:, k] is None:
            #         break
            #     et_kg_merge_entity_all = torch.cat([et_kg_merge_entity_all, et_kg_merge[:, k].unsqueeze(1)], dim=1)

            # sample_et_content_cluster_index = torch.zeros(sample_et_content_cluster.shape).cuda()
            # # 对每行进行处理
            # for aa in range(sample_et_content_cluster.size(0)):
            #     clu_unique_values, clu_unique_counts = torch.unique(sample_et_content_cluster[aa], return_counts=True)
            #     clu_unique_values_to_set_to_one = clu_unique_values[clu_unique_counts <= 2]
            #     for bb, cc in zip(sample_et_content_cluster[aa], range(0, sample_et_content_cluster.size(1))):
            #         if torch.eq(bb, clu_unique_values_to_set_to_one).any():
            #             sample_et_content_cluster_index[aa][cc] = 1
            #         else:
            #             sample_et_content_cluster_index[aa][cc] = 0
            #
            # sample_kg_content_relation_index = torch.zeros(sample_kg_content_relation.shape).cuda()
            # # 对每行进行处理
            # for a in range(sample_kg_content_relation.size(0)):
            #     unique_values, unique_counts = torch.unique(sample_kg_content_relation[a], return_counts=True)
            #     unique_values_to_set_to_one = unique_values[unique_counts <= 4]
            #     for b, c in zip(sample_kg_content_relation[a], range(0, sample_kg_content_relation.size(1))):
            #         if torch.eq(b, unique_values_to_set_to_one).any():
            #             sample_kg_content_relation_index[a][c] = 1
            #         else:
            #             sample_kg_content_relation_index[a][c] = 0

            '''
            sample_kg_content_relation_index = torch.zeros(sample_kg_content_relation.shape).cuda()
            unique_values, unique_counts = torch.unique(sample_kg_content_relation.view(-1), return_counts=True)
            unique_values_to_set_to_one = unique_values[unique_counts == 1]
            for a in range(sample_kg_content_relation.size(0)):
                for b, c in zip(sample_kg_content_relation[a], range(0, sample_kg_content_relation.size(1))):
                    if torch.eq(b, unique_values_to_set_to_one).any():
                        sample_kg_content_relation_index[a][c] = 1
            ...
            # sample_et_content_cluster_index = sample_et_content_cluster_index.unsqueeze(-1).expand(-1, -1, self.embedding_dim)
            # et_kg_merge_type_all = et_kg_merge_type_all * sample_et_content_cluster_index
            # et_kg_merge_type_add = torch.stack([et_kg_merge_type, et_kg_merge_type_all], dim=2)
            # et_kg_merge_type_add = et_kg_merge_type_add.mean(2)
            #
            # sample_kg_content_relation_index = sample_kg_content_relation_index.unsqueeze(-1).expand(-1, -1, self.embedding_dim)
            # et_kg_merge_entity_all = et_kg_merge_entity_all * sample_kg_content_relation_index
            # et_kg_merge_global = torch.stack([et_kg_merge_entity, et_kg_merge_entity_all], dim=2)
            # et_kg_merge_global = et_kg_merge_global.mean(2)
            # et_kg_merge_global = torch.cat([et_kg_merge_type, et_kg_merge_global], dim=1)
        else:
            et_kg_merge_type = None
            et_kg_merge_entity = None
            # et_kg_merge_global = None
        ...
        output, context_embedding, second_local_type, second_local_entity = self.layer(local_embedding, et_kg_merge, et_neighbor_size, flag) # et_kg_merge_global
...
class TETLayer(nn.Module):
    ...
    def forward(self, local_embedding, et_kg_merge, et_neighbor_size, flag): # et_kg_merge_global
        ...
        # shape(3, 100)
        pos = self.pos_embeds(torch.arange(0, 2).to(self.device)) # arange(0, 2)
        ...
        # if flag:
        #     attention_mask_two = torch.ones(batch_size, neighbor_size + 2).bool().to(self.device)
        #     second_global = torch.cat([self.global_cls.expand(batch_size, 1, emb_size), local_embedding, et_kg_merge], dim=1)
        #     # pos = self.pos_embeds(torch.arange(0, 3).to(self.device))  # arange(0, 2)
        #     second_global[:, 0] = second_global[:, 0] + pos[0].unsqueeze(0)
        #     second_global[:, 1:-1] = second_global[:, 1:-1] + pos[1].view(1, 1, -1)
        #     second_global[:, -1] = second_global[:, -1] + pos[2].unsqueeze(0)
        #     second_global = self.layer_norm(second_global)
        #     second_global = self.transformer_encoder(second_global, None, self.convert_mask_trm(attention_mask_two))
        #     second_global = second_global[-1]
        #
        #     second_global_type = torch.empty(0, device=self.device)
        #     second_global_entity = torch.empty(0, device=self.device)
        ...
        #             break
        #         second_global_type = torch.cat([second_global_type, second_global[:, i].unsqueeze(1)], dim=1)
        #     for j in range(4, neighbor_size + 1):
        #         if second_global[:, j] is None:
        #             break
        #         second_global_entity = torch.cat([second_global_entity, second_global[:, j].unsqueeze(1)], dim=1)
        #     second_global = second_global[:, 0].unsqueeze(1)
        # else:
        #     second_global_type = None
        #     second_global_entity = None
        # predict2 = self.fc(torch.nn.GELU()(second_global))
        # predict3 = self.fc(torch.nn.GELU()(et_kg_merge))  # (128, 1, 3584)
